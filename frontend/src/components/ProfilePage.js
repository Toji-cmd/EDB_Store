...
const ProfilePage = () => {
  ...
  const fetchUserData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BACKEND}user?token=${token}`
      );
      ...
      const productDetails = await Promise.all(
        historyArray.map(async (productId) => {
          const res = await fetch(
            `${process.env.REACT_APP_API_BACKEND}products/${productId}`
          );
  ...
  const updateUserData = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_BACKEND}user/update${updateQuery}`,
        {
          method: "POST",
        }
      );
